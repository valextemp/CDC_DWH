...
-- Create date: 20231011
-- Description:	Повторная загрузка из архивной таблицы [arсh].[asto_Rows_CT] для повторной обработки и загрузке в [stg].[asto_asto_Rows_CT]
-- =============================================

CREATE   PROCEDURE [stg].[usp_ReLoad_from_arch_asto_Rows_CT]
...
WHILE @RowCount>0
	...
	(
		SELECT 
			  ...
			  ,[HistoryUserId]
			  ,[__$command_id]
			  ,NULL as [LsnTime] -- пока так сделаю, буду в etl процессе вставлять хотя в архивной таблице уже есть время
			  --,[ASTO_ZF].sys.fn_cdc_map_lsn_to_time([__$start_lsn]) as [LsnTime]
		 FROM [arсh].[asto_Rows_CT]
	...
	END -- end цикла
		...
		IF @RowCountAll>0
			BEGIN
				...
				SELECT @Message=FORMATMESSAGE(N'[stg].[usp_ReLoad_from_arch_asto_Rows_CT]. Вставлено %i записей в [stg].[asto_Rows_CT]. Затрачено времени - %i мсек.',@RowCountAll, @TimeElapsed_s);
				EXEC [log].[usp_Add_Log_Event] @Message=@Message, @Level=N'Info'
			END
		ELSE
			IF @DebugInfo=1
				BEGIN
					SELECT @Message=FORMATMESSAGE(N'[stg].[usp_Load_asto_Rows_CT]. Нет записей для вставки в [stg].[asto_Rows_CT] . Затрачено времени - %i мсек.', @TimeElapsed_s);
...
-- EXEC [CDC_DWH].[stg].[usp_ReLoad_from_arch_asto_Rows_CT] 
